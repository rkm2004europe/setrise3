// lib/features/chat/screens/chat.dart
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _react(Message msg, String emoji) {
    ...
    if (emojiIdx != -1) {
      ...
      updated = r.mine
          ? List.from(existing)..removeAt(emojiIdx)
          : List.from(existing)
            ..[emojiIdx] = Reaction(emoji: emoji, count: r.count + 1, mine: true);
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // ── شريط علوي مخصص (بدون AppBar) ──
            GestureDetector(
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => ProfileScreen(user: widget.conversation.user),
                ),
              ),
              child: ChatAppBar(
                ...
                leading: IconButton(
                  icon: const Icon(Icons.arrow_back, color: ChatColors.text),
                  onPressed: () => Navigator.pop(context),
                ),
                actions: [
                  IconButton(
                    icon: const Icon(Icons.call, color: ChatColors.accent),
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => CallScreen(user: widget.conversation.user, isVideo: false),
                      ),
                    ),
                  ),
                  IconButton(
                    icon: const Icon(Icons.videocam, color: ChatColors.accent),
                    onPressed: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => CallScreen(user: widget.conversation.user, isVideo: true),
                      ),
                    ),
                  ),
                ],
                ...
                children: [
                  ListView.builder(
                    ...
                    itemBuilder: (ctx, i) => MessageBubble(
                      message: _msgs[i],
                      onLongPress: (m) => OptionsSheet.show(
                        context,
                        message: m,
                    ...
                    Positioned(
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Icon(Icons.keyboard_arrow_down, color: ChatColors.accent),
                        ),
                      ),
                    ),
                ],
