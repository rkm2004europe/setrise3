...
import '../widgets/gift_animation.dart'; // <-- استيراد أنيميشن الهدية
...
class _LiveRoomScreenState extends State<LiveRoomScreen> {
  ...
  String? _currentGiftEmoji; // <-- لتخزين إيموجي الهدية الحالية

  // ─── إرسال هدية (مع أنيميشن) ────────────────────────────────────
  void _sendGift(GiftModel gift) {
    HapticFeedback.mediumImpact();
    setState(() {
      _giftCtrl.sendGift(gift);
      _currentGiftEmoji = gift.animationEmoji; // عرض الأنيميشن
      _comments.insert(
        0,
        LiveCommentModel(
